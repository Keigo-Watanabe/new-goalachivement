  ...
  <app-layout>
    <div class="home-content">
      ...
      <div class="task-container">
        ...
        <!-- タスク絞り込みセクション -->
        <div class="task-nav-section">
          <div class="task-nav-box" v-bind:class="{ 'task-nav-responsive-open': isActive }">
            <div class="task-nav-responsive-close">
              <i class="fas fa-times" v-on:click="closeTaskNav"></i>
            </div>
            <!-- カテゴリー別 -->
            <div class="task-nav-category">
              <div class="task-nav-section-title">
                <span>カテゴリー別</span>
              </div>
              <ul class="task-nav-section-ul">
                <li class="task-nav-section-item" v-for="category in categories" v-bind:key="category.category_id" v-on:click="sortCategory(category.category_id)"><i class="fas fa-circle" v-bind:style="{ color: category.color }"></i>{{ category.category }}</li>
              </ul>
            </div>

            <!-- 優先度別 -->
            <div class="task-nav-priority">
              <div class="task-nav-section-title">
                <span>優先度別</span>
              </div>
              <ul class="task-nav-section-ul">
                <li class="task-nav-section-item" v-on:click="sortPriority">重要</li>
                <li class="task-nav-section-item" v-on:click="sortSeverity">緊急</li>
              </ul>
            </div>

            <!-- ステータス別 -->
            <div class="task-nav-status">
              <div class="task-nav-section-title">
                <span>ステータス別</span>
              </div>
              <ul class="task-nav-section-ul">
                <li class="task-nav-section-item" v-on:click="sortYet">未着手</li>
                <li class="task-nav-section-item" v-on:click="sortDoing">進行中</li>
                <li class="task-nav-section-item" v-on:click="sortDone">完了済</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div class="goal-task-list-section">
          <div class="goal-task-list-section-inner">
            <div class="goal-task-list-nav">
              ...
              <div class="task-list-responsive-bar">
                <i class="fas fa-bars" v-on:click="openTaskNav"></i>
              </div>

              <div class="goal-task-select">
                <button class="matrix-btn" v-on:click="taskMatrix"><i class="fas fa-th-large"></i>重要×緊急マトリクス</button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      sortDes() {
        this.$inertia.get('/task', { sort: '降順' })
        this.isDropdown = false;
      },

      sortAsc() {
        this.$inertia.get('/task', { sort: '昇順' })
        this.isDropdown = false;
      },

      sortCategory(category_id) {
        this.$inertia.get('/task', { sort: category_id })
        this.isDropdown = false;
      },

      sortPriority() {
        this.$inertia.get('/task', { sort: '重要' })
        this.isDropdown = false;
      },

      sortSeverity() {
        this.$inertia.get('/task', { sort: '緊急' })
        this.isDropdown = false;
      },

      sortYet() {
        this.$inertia.get('/task', { sort: '未着手' })
        this.isDropdown = false;
      },

      sortDoing() {
        this.$inertia.get('/task', { sort: '進行中' })
        this.isDropdown = false;
      },

      sortDone() {
        this.$inertia.get('/task', { sort: '完了済' })
        this.isDropdown = false;
      },
